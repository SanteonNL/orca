...
export default function EnrollmentTaskPage() {
    ...
    if (!task || !subTasks || !subTasks.length || !taskToQuestionnaireMap || !subTasks?.[0].id || !taskToQuestionnaireMap[subTasks[0].id]) {
        return <></>
    }
    ...
    switch (task.status) {
        ...
        default:
            ...
            return <p>Task status {task.status} is geen valide status voor een enrollment Task.</p>
    }
}
