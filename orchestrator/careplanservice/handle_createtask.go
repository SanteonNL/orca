...
func (s *Service) handleCreateTask(ctx context.Context, request FHIRHandlerRequest, tx *coolfhir.BundleBuilder) (FHIRHandlerResult, error) {
	...
	if task.BasedOn == nil || len(task.BasedOn) == 0 {
		...
		if task.For.Reference == nil {
			...
			patients, _, err := handleSearchResource[fhir.Patient](s, ctx, "Patient", map[string][]string{"identifier": {fmt.Sprintf("%s|%s", *task.For.Identifier.System, *task.For.Identifier.Value)}}, &headers)
			if err != nil {
				return nil, err
			}
