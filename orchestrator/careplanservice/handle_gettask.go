...
func (s *Service) handleGetTask(ctx context.Context, request FHIRHandlerRequest, tx *coolfhir.BundleBuilder) (FHIRHandlerResult, error) {
	log.Ctx(ctx).Info().Str("taskId", request.ResourceId).Msg("Handling get task request")

	// fetch Task + CareTeam, validate requester is participant of CareTeam
	var task fhir.Task
	err := s.fhirClient.ReadWithContext(ctx, "Task/"+request.ResourceId, &task)
	if err != nil {
		return nil, err
	}
	...
	return func(txResult *fhir.Bundle) (*fhir.BundleEntry, []any, error) {
		var returnedTask fhir.Task
		result, err := coolfhir.NormalizeTransactionBundleResponseEntry(ctx, s.fhirClient, s.fhirURL, &tx.Entry[0], &txResult.Entry[0], &returnedTask)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to get Task: %w", err)
		}

		return result, []any{&returnedTask}, nil
	}, nil
}
