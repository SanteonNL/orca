...
func (s *Service) handleSearchCarePlan(ctx context.Context, queryParams url.Values, headers *fhirclient.Headers) (*fhir.Bundle, error) {
	...
	for _, ct := range careTeams {
		...
		careTeamIDs = append(careTeamIDs, *ct.Id)
	...
	filterMissingResourcesOutOfBundle(&bundle, "CareTeam", careTeamIDs)
	filterMissingResourcesOutOfBundle(&bundle, "CarePlan", carePlanIDs)
