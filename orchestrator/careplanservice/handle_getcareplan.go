...
func (s *Service) handleGetCarePlan(ctx context.Context, request FHIRHandlerRequest, tx *coolfhir.BundleBuilder) (FHIRHandlerResult, error) {
	...
	err := s.fhirClient.ReadWithContext(ctx, "CarePlan/"+request.ResourceId, &carePlan)
	if err != nil {
		return nil, err
	}
	...
	return func(txResult *fhir.Bundle) (*fhir.BundleEntry, []any, error) {
		var returnedCarePlan fhir.CarePlan
		result, err := coolfhir.NormalizeTransactionBundleResponseEntry(ctx, s.fhirClient, s.fhirURL, &tx.Entry[0], &txResult.Entry[0], &returnedCarePlan)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to get CarePlan: %w", err)
		}

		return result, []any{&returnedCarePlan}, nil
	}, nil
}
