...
//
//	mockgen -destination=./policy_agent_mock.go -package=policy -source=agent.go Agent
//
...
func (m *MockPolicyAgent) Allow(ctx context.Context, context *Context) error {
	m.ctrl.T.Helper()
...
func (m *MockPolicyAgent) Preflight(resourceType, id string, r *http.Request) (*Preflight, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Preflight", resourceType, id, r)
	ret0, _ := ret[0].(*Preflight)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
...
func (m *MockPolicyAgent) PrepareContext(ctx context.Context, cache SearchCache, preflight *Preflight, resource any) (*Context, error) {
	m.ctrl.T.Helper()
