...
func WithAuditEvent(ctx context.Context, t *BundleBuilder, info AuditEventInfo) BundleEntryPostOption {
	return func(entry *fhir.BundleEntry) {
		...
		if entry.FullUrl != nil {
			...
			resourceRef.Type = to.Ptr(res.Type)
			log.Ctx(ctx).Debug().Str("resourceType", res.Type).Str("resourceId", res.ID).Msg("Created resource reference for audit event")
		} else {
			log.Ctx(ctx).Error().Str("resourceType", res.Type).Msg("Unable to create proper reference for audit event, missing both FullUrl and resource ID")
			// TODO: Find some way to fail the bundle creation
			t.Append(poisonPill{}, nil, nil)
			return
		}
