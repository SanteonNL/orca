...
func TestResolveCareTeam(t *testing.T) {
	tests := map[string]struct {
		...
		"careteam not found": {
			...
			expectedErr: "CareTeam not found",
		},
	}

	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			result, err := ResolveCareTeam(&tt.carePlan)
...
func TestUpdateContainedResource(t *testing.T) {
	t.Run("updating works", func(t *testing.T) {
		id := fhir.Reference{Reference: to.Ptr("#careteam")}
		carePlan := fhir.CarePlan{
		...
		contained, err := UpdateContainedResource(carePlan.Contained, &id, "CareTeam", expected)
