...
func ImportResources(ctx context.Context, client fhirclient.Client, resourceTypes []string, urlToLoad string) (int, error) {
	var bundleData []byte
	fileUrlPrefix := "file://"
	if strings.HasPrefix(urlToLoad, fileUrlPrefix) {
		...
		if httpResponse.StatusCode <= 199 || httpResponse.StatusCode >= 300 {
			return 0, fmt.Errorf("unexpected status code: %d", httpResponse.StatusCode)
		}
	...
	if resourceDesc, err := fhirclient.DescribeResource(bundleData); err != nil {
		...
		return 0, fmt.Errorf("not a FHIR Bundle: %s", resourceDesc.Type)
	}
	var bundle fhir.Bundle
	if err := json.Unmarshal(bundleData, &bundle); err != nil {
		return 0, fmt.Errorf("could not unmarshal bundle: %w", err)
	}
	...
	for _, entry := range bundle.Entry {
		...
		if strings.Contains(entry.Request.Url, "://") {
			return 0, fmt.Errorf("entry contains a full URL, which is not supported: %s", entry.Request.Url)
		}
		...
		if !slices.Contains(resourceTypes, resourceType) {
			return 0, fmt.Errorf("entry contains a resource of an unexpected type: %s", resourceType)
		}
	}

	// Post the Bundle to the FHIR server
	var httpStatusCode int
	if err := client.CreateWithContext(ctx, bundle, nil, fhirclient.ResponseStatusCode(&httpStatusCode)); err != nil {
