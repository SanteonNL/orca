...
func (s *Service) handleTaskNotification(ctx context.Context, cpsClient fhirclient.Client, task *fhir.Task) error {
	...
	identifiers := profile.IdentifiersOf(identities)
	...
	if partOfRef == nil {
		...
		isOwner, _ := coolfhir.IsIdentifierTaskOwnerAndRequester(task, identifiers)
		if !isOwner {
		...
		err = s.createSubTaskOrAcceptPrimaryTask(ctx, cpsClient, task, task, identifiers)
		if err != nil {
		...
		err = s.handleSubtaskNotification(ctx, cpsClient, task, *partOfRef, identifiers)
		if err != nil {
